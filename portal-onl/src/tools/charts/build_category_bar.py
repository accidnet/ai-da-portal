...
def tool_definition() -> dict[str, object]:
    """category bar chart 생성 tool 정의를 반환합니다."""
    return {
        "type": "function",
        "name": "build_category_bar",
        "description": "범주/숫자 조합 또는 기본 행 분포를 bar chart payload로 생성합니다.",
        "parameters": {
            "type": "object",
            "properties": {
...
def execute(arguments: dict[str, object]) -> dict[str, object]:
    """LLM function_call arguments만 받아 category bar chart를 생성합니다."""
    try:
        dataset_id, dataframe = load_dataframe_from_arguments(arguments)
        chart = build_category_bar(dataframe)
    except KeyError:
