...
def tool_definition() -> dict[str, object]:
    """insight payload 생성 tool 정의를 반환합니다."""
    return {
        "type": "function",
        "name": "build_insight",
        "description": "분석 유형과 prompt에 맞는 기본 insight payload를 생성합니다.",
        "parameters": {
            "type": "object",
            "properties": {
...
def execute(arguments: dict[str, object]) -> dict[str, object]:
    """LLM function_call arguments만 받아 insight payload를 생성합니다."""
    try:
    ...
    return tool_success({"dataset_id": dataset_id, "insight": insight.model_dump(mode="json")})
