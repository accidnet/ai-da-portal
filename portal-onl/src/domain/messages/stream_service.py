...
class MessageStreamService:
    ...
    async def create_streaming_response(
        self, *, request: Request, agent_runtime: ChatStreamingAgent
    ) -> StreamingResponse:
        try:
            payload, uploaded_file = await self._parse_stream_request(request)
            payload, interaction_dataset = await self._message_service.prepare_chat_request(
        ...
        def event_stream() -> Generator[str, None, None]:
            try:
                ...
                for event in self._message_service.stream_chat(
                    payload=payload,
    ...
    ) -> tuple[ChatRequest, UploadFile | None]:
            ...
            try:
                return (
                    ChatRequest(
                        ...
                        message=message,
                        dataset_ids=self._parse_dataset_ids(dataset_ids_json),
                    ),
                    file,
                )
    ...
    def _parse_dataset_ids(self, dataset_ids_json: str | None) -> list[str]:
