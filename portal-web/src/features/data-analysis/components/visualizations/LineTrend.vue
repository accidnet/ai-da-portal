...
import { resolveLinePath, resolveSeriesPoints } from "./chartUtils";
...
const points = computed(() =>
  resolveSeriesPoints(props.chart, props.fallbackPoints),
);
  ...
  <div class="viz-shell">
    <div class="viz-body">
      ...
      >
        <path
          class="viz-grid"
          d="M 0 85 L 100 85 M 0 55 L 100 55 M 0 25 L 100 25"
        ></path>
        <path class="viz-line" :d="linePath"></path>
      </svg>
      <div v-if="hasData" class="viz-labels">
        <small v-for="point in points" :key="point.label">{{
          point.label
        }}</small>
      </div>
